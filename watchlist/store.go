...
// sessionIDPattern matches only the hex session IDs generated by the server.
...
var ErrInvalidSessionID = errors.New("watchlist: invalid session ID")
...
func (fst *FileStore) filePath(sessionID string) (string, error) {
	if !sessionIDPattern.MatchString(sessionID) {
		return "", ErrInvalidSessionID
	}
	return filepath.Join(fst.dataDir, sessionID+".json"), nil
}

func (fst *FileStore) readItems(sessionID string) []Item {
	path, err := fst.filePath(sessionID)
	if err != nil {
		log.Printf("watchlist: readItems: %v", err)
		return []Item{}
	}
...
func (fst *FileStore) Clear(sessionID string) {
	...
	path, err := fst.filePath(sessionID)
	if err != nil {
		log.Printf("watchlist: clear: %v", err)
		return
	}
